...
test("landing presents the honest delivery and qualification contract", async ({ page }) => {
  ...
  await expect(page.getByLabel("Current public status")).toContainText("T75 verified");
  await expect(page.getByLabel("Current public status")).toContainText("T76 next");
...
test("deep routes, roadmap links, community calls to action, and recovery resolve under the base path", async ({
  ...
  // hidden variant is first in the DOM, so target the visible T75 text directly
  // rather than whichever variant happens to come first.
  await expect(page.getByText(/T75/u).and(page.locator(":visible")).first()).toBeVisible();
