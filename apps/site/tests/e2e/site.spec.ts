...
test("landing presents the honest delivery and qualification contract", async ({ page }) => {
  ...
  await expect(page.getByLabel("Current public status")).toContainText("T68b verified");
  await expect(page.getByLabel("Current public status")).toContainText("T68c next");
