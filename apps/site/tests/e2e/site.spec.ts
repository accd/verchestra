...
test("landing presents the honest delivery and qualification contract", async ({ page }) => {
  ...
  await expect(page.getByLabel("Current public status")).toContainText("T72 verified");
  await expect(page.getByLabel("Current public status")).toContainText("T73 next");
...
test("deep routes, roadmap links, community calls to action, and recovery resolve under the base path", async ({
  ...
  // hidden variant is first in the DOM, so target the visible T72 text directly
  // rather than whichever variant happens to come first.
  await expect(page.getByText(/T72/u).and(page.locator(":visible")).first()).toBeVisible();
