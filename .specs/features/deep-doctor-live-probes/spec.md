...
- [x] Seven of seven checks report from live observation; five unchanged.
      **`secret-presence` (T15) is complete, post-review (2026-08-22)** —
      originally deferred as AD-023 because no `OsSecretBackend` (Windows CNG
      / Apple Keychain / Linux Secret Service) implementation exists anywhere
      in the repository. Reconciling this branch with an independently landed
      competing #207 implementation on `main` surfaced a working live-adapter
      wiring; adopted here through this repository's own
      `@verchestra/platform-node/readonly` subpath (T10's `secretPresence`)
      rather than building an `OsSecretBackend` speculatively — the caller
      still supplies the adapter through `DoctorLiveProbeOptions.secret`, and
      no backend is fabricated. See AD-028's superseded note in
      `.specs/STATE.md`.
- [x] `pnpm gate:quick` and `pnpm test:architecture` pass with no skipped or weakened assertion.
- [x] The transitive guard fails when a writer is introduced into the closure (discrimination sensor).
- [ ] A T75 matrix leg records a sealed report in which all seven checks are not `blocked` — needs
      T22's human-triggered CI run; not yet captured.
