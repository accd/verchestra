...
#### T15: Live secret-presence probe — COMPLETE (2026-08-22, post-review)
**What**: Replace the file probe with the read-only has-surface.
**Where**: `apps/vestra-cli/src/doctor-composition.ts` (`secretPresenceProbe`)
**Depends on**: T5, T7, T10 · **Requirement**: DDL-09 · **AC**: 12
**Status**: Complete. `doctor.secret-presence` now calls
`secretPresence(live.secret.adapter, live.workspaceId, live.secret.logicalName)`
(the T10 readonly-subpath wrapper), reporting `blocked` whenever no live
secret port is configured.

> **Originally deferred, superseded during PR #306's review response
> (2026-08-22).** Reconciling this branch with an independently landed
> competing #207 implementation on `main` (`cf8913c`, `f901b0f`) surfaced a
> working `DoctorLiveProbeOptions.secret`/`secretPresenceProbe` wiring built
> there. Adopted here, rerouted through this repository's own
> `@verchestra/platform-node/readonly` subpath instead of the competing
> branch's direct `@verchestra/platform-node` import, so the doctor's
> transitive read-only closure guard (T11/AD-025) still holds; narrowed
> `secretPresence`'s parameter type from `SecretAdapter` to
> `Pick<SecretAdapter, "has">` so the port stays structurally unable to reach
> `.read()`. The original deferral's finding — no `QualifiedOsSecretAdapter`/
> `OsSecretBackend` implementation exists for any platform — still applies to
> what a caller *can* pass; nothing here fabricates one. See the historical
> reasoning below and AD-028's superseded note in `.specs/STATE.md`.
...
| T15 | Done (completed post-review, PR #306) | recorded in `handoff.md`; AD-028 superseded |
