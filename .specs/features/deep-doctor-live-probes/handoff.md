...
T1 through T21 are complete and gated on branch. Phases 2, 3, 4, and 5 fully
done; Phase 6 done except T22, which needs a human-triggered T75 CI run this
session cannot execute. **T15 (secret-presence live wiring) is no longer
deferred** — see the PR #306 review-response entry below: reconciling with an
independently landed competing #207 implementation on `main` surfaced a
working live-adapter wiring, adopted here through this repository's own
readonly-subpath convention. PR #306 was changed from `Closes #207` to
`Refs #207` because T22 (T75 fleet evidence) is the one remaining acceptance
gap and needs a human trigger this session cannot provide.
...
- **T11** — `tests/architecture/doctor-readonly-graph.test.mjs` gained a
  ...
  demonstrated, not just claimed; (2) an entry-file import of the _allowed_
  `@verchestra/platform-node/readonly` subpath passes; (3) an entry-file
  import of the _forbidden_ bare `@verchestra/platform-node` root fails both
...
- **T13** — `apps/vestra-cli/src/doctor-composition.ts`'s
  ...
  **One more finding, this time about the spec's own edge case.**
  ...
  degrade _any_ injected error (a lock-shaped one included) to `fail`, never
...
- **PR #306 review response — merge with `main` plus three P1 fixes
  (2026-08-22).** Two rounds of independent review landed while this PR was
  open. First round found the merge conflict and a SonarCloud `jssecurity:S8707`
  failure; fixed by canonicalizing and validating `controlRoot` before any
  fixture write (`e4b1592`). Second round, on that head, found three further
  issues, all addressed in this entry's commit:
  1. **Merge conflict against `main`.** `main` had independently landed a
     competing, overlapping #207 implementation
     (`.specs/features/live-doctor-probes/`, `cf8913c`/`f901b0f`/`eca4052`)
     touching the same files (`doctor-composition.ts`,
     `doctor-readonly-graph.test.mjs`, `doctor-facts.ts`). Its design differs:
     driver/connector checks construct a real `PiDriver`/adapter directly
     rather than reading an availability record, which this branch's own
     transitive-closure guard (AD-025/T11) forbids. Resolved by keeping this
     branch's more conservative, already-reviewed design (availability
     records for driver/connector/probe; the closure guard unchanged) and
     adopting the one genuinely new capability the other branch had that
     this one lacked: a working `secretPresenceProbe`/`DoctorLiveProbeOptions.secret`
     wiring — T15, previously deferred as AD-023 (see below). Rerouted
     through this repository's own `@verchestra/platform-node/readonly`
     subpath (`secretPresence`, T10) instead of the competing branch's direct
     `@verchestra/platform-node` import, narrowing that function's parameter
     type from `SecretAdapter` to `Pick<SecretAdapter, "has">` so the port
     stays structurally unable to reach `.read()`. AD-028's deferral is
     superseded, not erased — the STATE.md entry records why.
  2. **[P1] Cedar-policy trust anchor (review finding, `doctor-composition.ts:230`).**
     `cedarPolicyReadOnlyCrypto`'s `verify` used the bundle's own declared
     `publicKeyRef` as the verification key — a fully replaced,
     self-consistent, self-signed bundle (content, digest, signature, and key
     all swapped together) would pass. Fixed: the provisioner now writes a
     sibling `trusted-signer.pub` file alongside `active.bundle`, and the
     probe reads it independently and pins verification to that value,
     ignoring the bundle's own `publicKeyRef` parameter entirely. A bundle
     present without its trust anchor reports `blocked`, not `pass`.
     Regression: `tests/integration/doctor-cedar-policy-probe.test.mjs` — "a
     fully replaced, self-consistent, self-signed bundle reports fail, not
     pass" (constructs a real untrusted keypair, re-signs a full replacement
     bundle with `buildPolicyBundle`, confirms `fail`) and "a bundle present
     without its trust anchor reports blocked, never pass".
  3. **[P1] Provisioner descendant-path containment (review finding,
     `scripts/provision-doctor-fixtures.mjs:75`).** Canonicalizing only
     `controlRoot` does
     not contain `.verchestra` or a subsystem directory below it (e.g.
     `sandbox`) if either already exists as a symlink/junction planted before
     the call — every write beneath it could then land outside the requested
     root. Fixed with `mkdirContained`/`writeFileContained`/
     `assertNotExistingRedirect` helpers: every mkdir, writeFile, rm, and
     symlink target is now routed through a walk that creates missing path
     segments and refuses, before any write reaches it, the moment an
     existing segment is not a genuine directory (or, for a file target, is
     itself a symlink or hard link). Regressions in
     `tests/integration/provision-doctor-fixtures.test.mjs`: pre-planting a
     symlink at the metadata root, at a subsystem directory (`sandbox`), and
     at a file target (`active.bundle`) are each refused before any write
     reaches through them. The static S8707 findings are addressed at the
     sink boundary: the qualification root is rebuilt from a trusted OS temp
     directory entry, and each remaining filesystem call has a co-located
     containment/redirect check. SonarCloud now reports zero open S8707
     findings without suppressions.
  4. **[P1] `Closes #207` while acceptance was incomplete (review finding).**
     Changed PR #306 from `Closes #207` to `Refs #207`; T22 (T75 fleet
     evidence, needs a human-triggered CI run) is the one remaining
     acceptance gap now that T15 is done.
  5. **Follow-up Sonar and portability correction (2026-08-23).** The
     provisioner now validates the caller root before canonicalization, uses
     the canonical path returned by each containment check, refuses to
     recursively replace a normal pre-existing escape directory, and rejects
     hard-link file redirects as well as symlinks. The Windows regression uses
     a hard link because unprivileged file symlinks are unavailable by default;
     POSIX continues to exercise a file symlink. A disposable mutation that
     removed hard-link rejection caused the regression to fail, so the sensor
     is discriminating. Gates: `pnpm gate:full` PASS and `pnpm gate:security`
     PASS; all focused doctor/provision cases pass with zero failed, skipped,
     or todo outcomes. A subsequent S8707 correction (commit `9e08b63`)
     restricts CLI roots to canonical children of the OS temp directory and
     reconstructs them from the trusted root before any sink; the focused
     provisioner suite remains 9/9 with zero failed, skipped, or todo
     outcomes.
...
- **T19** — `doctor.probe` reuses T17's `availabilityProbe` unmodified; wrote
  the wiring-isolation test _first_ this time, applying T18's lesson
...
session can review its evidence, close T22, and change PR #306 from
...
- **Node environment fix (2026-08-22, outside repo scope but requested by the
  ...
  _this_ conversation by re-pointing `/opt/homebrew/bin/{node,npm,npx,corepack}`
