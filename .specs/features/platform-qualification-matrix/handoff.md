...
nextTask: "The evidence index generator (scripts/t75-evidence-index.mjs, pnpm t75:evidence-index) RECONCILES the declared matrix.json against the observed fleet indexes across BOTH fleet-answerable dimensions: platform (unit = leg) and gate-profile (unit = dispatch, judged against the legs the declaration expects green, so the environmental darwin-x64 absence does not mark every profile unexercised). Silence is not a pass in either; an observation never upgrades a declaration; disagreements are recorded as contradictions and drive a non-zero CLI exit. Two independent verification rounds FAILED earlier attempts (validation-evidence-index.md keeps both verbatim); 12/12 then 7/7 mutants now die. LIVE FINDING from the run at 9aab070 over the four dispatched profiles: 1 contradiction -- gate-profile/quick is declared qualified but NO dispatch at that revision covers it (matrix.md:75 already notes quick ran elsewhere), so T75 still needs a gate=quick fleet dispatch at the qualification revision before the index can certify that dimension. REMAINING: (1) that quick dispatch; (2) the SIGNATURE -- the index is deliberately unsigned and says so, AD-014 now carries it in scope, but WHICH IDENTITY signs release evidence is an open owner key-custody decision; (3) C5/#207 live doctor probes (bruno); (4) B3 independent t75-validation.md, which runs this generator at the qualification revision and commits evidence-index.json beside the report. Chain is at T73; T74 must qualify first."
