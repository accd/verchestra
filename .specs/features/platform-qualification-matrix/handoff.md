...
nextTask: "The evidence index GENERATOR exists (scripts/t75-evidence-index.mjs): it joins the declared matrix.json with the per-profile fleet indexes and emits the T75 record, with signingState explicitly UNSIGNED and the reason stated. Verified end to end against the four real fleet indexes at 9aab070 (52 cases: 41 qualified, 8 contract-qualified, 2 not-qualified, 1 environmental). REMAINING: (1) an owner key-custody decision for what identity signs qualification evidence -- a TEST-ONLY key would be a category error here; (2) C5/#207 live doctor probes (bruno); (3) B3 independent t75-validation.md, which runs this generator at the qualification revision and cites its output. Chain is at T73; T74 must qualify first."
