...
nextTask: "The evidence index generator (scripts/t75-evidence-index.mjs, pnpm t75:evidence-index) reconciles the declared matrix.json against the observed fleet across BOTH fleet-answerable dimensions -- platform (unit = leg) and gate-profile (unit = dispatch). It fails closed in three directions: silence is not a pass; an observation never upgrades a declaration; and a non-qualified declaration is contradicted when the fleet observes something it does not predict (environmental is discharged by missing, NEVER by failed -- otherwise the declaration under verification decides how strictly it is verified). Each profile records the excused legs its coverage claim was made under. THREE independent verification rounds failed earlier attempts; validation-evidence-index.md keeps all verdicts verbatim. LIVE FINDING at 9aab070 over the four dispatched profiles: 52 cases, 40 qualified, 1 contradiction -- gate-profile/quick is declared qualified but NO dispatch at that revision covers it (matrix.md:75 notes quick ran elsewhere), so T75 needs a gate=quick fleet dispatch at the qualification revision. REMAINING: (1) that quick dispatch; (2) the SIGNATURE -- deliberately unsigned and says so; AD-014 and the DSSE migration now carry it as a ninth kind with a reserved predicate URI, but WHICH IDENTITY signs release evidence is an open owner key-custody decision; (3) C5/#207 live doctor probes (bruno); (4) B3 independent t75-validation.md, which runs the generator at the qualification revision and commits evidence-index.json beside the report. Chain is at T73; T74 must qualify first."
