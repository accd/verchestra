...
nextTask: "The evidence index generator (scripts/t75-evidence-index.mjs, pnpm t75:evidence-index) RECONCILES the declared matrix.json against the observed fleet indexes: a platform case is recorded qualified only if every dispatched profile that covers it observed it qualified, and disagreements are recorded as contradictions in both directions. An independent verifier FAILED the first attempt -- see validation-evidence-index.md, kept verbatim -- because it concatenated instead of reconciling; that is fixed and 16/16 mutants now die. Run at 9aab070 over the four real fleet indexes: 52 cases, 41 qualified, 8 contract-qualified, 2 not-qualified, 1 environmental, 0 contradictions. REMAINING: (1) the SIGNATURE -- the index is deliberately unsigned and says so; AD-014 now carries it in scope so the DSSE migration seals it, but WHICH IDENTITY signs release evidence is still an open owner key-custody decision; (2) C5/#207 live doctor probes (bruno); (3) B3 independent t75-validation.md, which runs this generator at the qualification revision and cites its output. Chain is at T73; T74 must qualify first."
