...
| D1 | Done | Commit `a2990e0`. `apps/site/lighthouserc.cjs`: removed top-level `assert.aggregationMethod`; `"median"` on `categories:performance` only, `"pessimistic"` explicit on the other five. New test `apps/site/tests/unit/lighthouse-budget-aggregation.test.mjs` (8 cases) confirmed to fail against the prior global-`median` config (5 failures via `git stash`) before confirmed passing against the fix. Full evidence and the `optimistic`-vs-`median` measurement in the D1 evidence section above. |
| D2 | Done | Commit `b565470`. See the D2 evidence table above: `N = 11` pre-remedy CI sample via throwaway draft PR `brunomjanuario/verchestra#1` (closed, branch deleted). Median = 1, minimum = 0.92 — instability reaffirmed on the complete sample. |
| D3 | Done | Commits `38024c8` (upload step), `bde0e6b` (fix: `include-hidden-files: true` — the action's default silently excludes the dot-prefixed `.lighthouseci` directory). Confirmed on real CI run `30659193501`: `lighthouse-reports-0e73a2fce904cafd6a78888556803abc456afac2` artifact, 4,076,869 bytes, non-empty. |
| D4 | Done | Commits `d99a97c` (handoff.md + STATE.md restore + spec reconciliation), `bde0e6b` (fix: `lastCompletedTask: null` — `D3` failed the schema validator's `^T\d+$` constraint, since Phase D's task IDs are outside the numbered T-chain by design). `git diff main -- .specs/STATE.md` empty; `handoff.md` parses clean via `parseHandoff`. |
| D5 | Done | Independent Verifier, fresh standalone pass over `4c0ce07..bde0e6b` (19 commits, confirmed to include the PR's actual head via `gh pr view 139 --json headRefOid`). **PASS** on all 12 ACs, superseding the void `b91eea4` PASS. Full report: `validation.md`. (Two prior dispatch attempts hit infrastructure errors — API connection drop, agent stall — before finding anything; the third completed cleanly.) |
