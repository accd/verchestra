...
// Accepts only the "has" member, not the full adapter, so a caller wiring
// this into a read-only port (deep doctor's DoctorLiveProbeOptions) can
// never structurally acquire `read` through this function's parameter type.
export async function secretPresence(
  adapter: Pick<SecretAdapter, "has">,
  workspaceId: string,
  logicalName: string
): Promise<boolean> {
