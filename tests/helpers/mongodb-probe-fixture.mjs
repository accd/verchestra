import {
  DatabaseRegistry,
  MemoryDatabaseRegistrationStore,
  ProbePlanner
} from "../../packages/data-probe/src/index.ts";
import {
  MongoDbFixtureConnection,
  MongoDbProbeAdapter,
  parseMongoDbReadOperation
} from "../../packages/data-probe/src/mongodb-adapter.ts";
import {
  MemoryProbeResultSink,
  MemoryProtectedParameterBroker,
  ProbeWorkerSupervisor
} from "../../packages/extension-host/src/index.ts";
import { policy, registration, request } from "./database-probe-fixture.mjs";

export const findCommand = (overrides = {}) => ({
  kind: "find",
  database: "sales",
  collection: "orders",
  filter: { status: { $eq: { $param: 0 } } },
  projection: { _id: 1, status: 1 },
  sort: { _id: 1 },
  ...overrides
});

export async function mongoDbFixture(options = {}) {
  const command = options.command ?? findCommand();
  const classifications = options.parameterClassifications ?? (command.kind === "introspect" ? [] : ["internal"]);
  const protectedRequestRef = request().operation.protectedRequestRef;
  const operation = parseMongoDbReadOperation(command, {
    protectedRequestRef,
    parameterClassifications: classifications
  });
  const registry = new DatabaseRegistry({ store: new MemoryDatabaseRegistrationStore() });
  await registry.register(registration({ engine: "mongodb", approvedSchemas: ["sales"] }));
  const planner = new ProbePlanner({
    registry,
    adapters: [{ engine: "mongodb", validateNormalizedOperation: () => [] }]
  });
  const plan = await planner.plan(
    request({ operation, bounds: options.bounds ?? request().bounds }),
    policy({
      allowedObjects: ["sales.orders", "sales.collections_catalog"],
      allowedFunctions: [
        "and",
        "eq",
        "exists",
        "explain",
        "gt",
        "gte",
        "in",
        "limit",
        "lt",
        "lte",
        "match",
        "ne",
        "or",
        "project",
        "sort"
      ],
      allowCatalogAccess: command.kind === "introspect",
      ...options.policy
    })
  );
  const parameters = new MemoryProtectedParameterBroker();
  parameters.set(
    protectedRequestRef,
    new TextEncoder().encode(
      JSON.stringify({
        schemaVersion: 1,
        command,
        parameters: options.parameters ?? (classifications.length ? ["paid"] : [])
      })
    )
  );
  const results = new MemoryProbeResultSink();
  const connection = options.realConnection ?? new MongoDbFixtureConnection(options.connection);
  const worker = new MongoDbProbeAdapter({ connection });
  const supervisor = new ProbeWorkerSupervisor({
    worker,
    parameters,
    results,
    plan,
    expectedComponent: MongoDbProbeAdapter.component,
    maximumMessageBytes: 65_536
  });
  return { command, operation, plan, parameters, results, connection, worker, supervisor };
}
